import tensorflow as tf
from pathlib import Path
import mlflow
import mlflow.keras
from urllib.parse import urlparse
from NephroVision import logger

from NephroVision.entity.config_entity import EvaluationConfig
from NephroVision.utils.common import save_json

import dagshub

dagshub.init(
    repo_owner="Piyushgaur26",
    repo_name="NephroVision-DeepLearning-Based-Kidney-Disease-Analyzer",
    mlflow=True,
)


class Evaluation:
    def __init__(self, config: EvaluationConfig):
        self.config = config

    def _valid_generator(self):
        datagenerator_kwargs = dict(rescale=1.0 / 255, validation_split=0.30)

        dataflow_kwargs = dict(
            target_size=self.config.params_image_size[:-1],
            batch_size=self.config.params_batch_size,
            interpolation="bilinear",
        )

        valid_datagenerator = tf.keras.preprocessing.image.ImageDataGenerator(
            **datagenerator_kwargs
        )

        self.valid_generator = valid_datagenerator.flow_from_directory(
            directory=self.config.training_data,
            subset="validation",
            shuffle=False,
            **dataflow_kwargs,
        )

    @staticmethod
    def load_model(path: Path) -> tf.keras.Model:
        return tf.keras.models.load_model(path)

    def evaluation(self):
        self.model = self.load_model(self.config.path_of_model)
        self._valid_generator()
        self.score = self.model.evaluate(self.valid_generator)
        self.save_score()

    def save_score(self):
        scores = {"loss": self.score[0], "accuracy": self.score[1]}
        save_json(path=Path("scores.json"), data=scores)

    def log_into_mlflow(self):
        # Set MLflow tracking URI (DagsHub)
        logger.info("🚀 Starting MLflow logging process...")
        mlflow.set_tracking_uri(self.config.mlflow_uri)

        logger.info(f"MLflow Tracking URI set to: {self.config.mlflow_uri}")
        tracking_url_type_store = urlparse(mlflow.get_tracking_uri()).scheme

        with mlflow.start_run(run_name="Model_Evaluation") as run:
            logger.info(f"MLflow Run started with run_id: {run.info.run_id}")
            # ---- Params ----
            mlflow.log_params(self.config.all_params)

            logger.info("✅ Parameters logged to MLflow")
            # ---- Metrics ----
            mlflow.log_metric("loss", self.score[0])
            mlflow.log_metric("accuracy", self.score[1])

            # ---- Model Logging ----
            if tracking_url_type_store != "file":
                mlflow.keras.log_model(
                    self.model,
                    artifact_path="model",
                    registered_model_name="VGG16Model",
                )
            else:
                mlflow.keras.log_model(self.model, artifact_path="model")
        logger.info("🎯 MLflow logging completed successfully!")
